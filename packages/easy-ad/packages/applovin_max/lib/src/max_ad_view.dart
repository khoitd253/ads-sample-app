...
class MaxBannerAdViewController extends ChangeNotifier {
  /// Loads a native ad.
  void loadAd() {
    notifyListeners();
  }
...
class MaxAdView extends StatefulWidget {
  ...
  /// The controller that reloads a banner ad.
  final MaxBannerAdViewController? controller;
...
class _MaxAdViewState extends State<MaxAdView> {
  ...
  void initState() {
    ...
    _adaptiveBannerEnabled =
        (widget.extraParameters?['adaptive_banner'] == 'true');
  ...
  void _onMaxAdViewCreated(int id) {
    ...
    _methodChannel!.setMethodCallHandler((call) async {
      ...
      if ("OnAdViewAdLoadedEvent" == method) {
        ...
        widget.listener
            ?.onAdLoadedCallback(AppLovinMAX.createAd(adUnitId, arguments));
        ...
        widget.listener
            ?.onAdExpandedCallback(AppLovinMAX.createAd(adUnitId, arguments));
      } else if ("OnAdViewAdCollapsedEvent" == method) {
        widget.listener
            ?.onAdCollapsedCallback(AppLovinMAX.createAd(adUnitId, arguments));
  ...
  bool _isTablet() {
    ...
    final ui.Size size = PlatformDispatcher.instance.views.first.physicalSize;
