class AdmobTestId {
  static const String admobAppId = 'ca-app-pub-3940256099942544~3347511713';

  /// Ad_Resume
  static const String admobResumeId = 'ca-app-pub-3940256099942544/9257395921';

  /// Ad_Banner
  static const String admobBannerId = 'ca-app-pub-3940256099942544/6300978111';

  /// Ad_Banner_Collapse
  static const String admobBannerCollapseId = 'ca-app-pub-3940256099942544/2014213617';

  /// Ad_Interstitial
  static const String admobInterstitialId = 'ca-app-pub-3940256099942544/1033173712';

  /// Ad_Interstitial_Video
  static const String admobInterstitialVideoId = 'ca-app-pub-3940256099942544/8691691433';

  /// Ad_Native
  static const String admobNativeId = 'ca-app-pub-3940256099942544/2247696110';

  /// Ad_Native_Video
  static const String admobNativeVideoId = 'ca-app-pub-3940256099942544/1044960115';

  /// Ad_reward
  static const String admobRewardId = "ca-app-pub-3940256099942544/5224354917";
}
